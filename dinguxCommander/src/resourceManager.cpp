...
void CResourceManager::onResize()
{
    if (screen.ppu_x != m_ppu_x || screen.ppu_y != m_ppu_y) {
        m_surfaces[T_SURFACE_FOLDER] = LoadIcon(ResPath("folder.png"));//m_surfaces[T_SURFACE_FOLDER] = LoadIcon(ResPath("skin/icon-folder.png"));//
        m_surfaces[T_SURFACE_FILE] = LoadIcon(ResPath("file-text.png"));//m_surfaces[T_SURFACE_FILE] = LoadIcon(ResPath("skin/icon-font-size.png"));//
        m_surfaces[T_SURFACE_FILE_IMAGE] = LoadIcon(ResPath("file-image.png"));//m_surfaces[T_SURFACE_FILE_IMAGE] = LoadIcon(ResPath("skin/icon-brightness-48.png"));//
        m_surfaces[T_SURFACE_FILE_INSTALLABLE_PACKAGE] = LoadIcon(ResPath("file-ipk.png"));//m_surfaces[T_SURFACE_FILE_INSTALLABLE_PACKAGE] = LoadIcon(ResPath("skin/fixit.png"));//
        m_surfaces[T_SURFACE_FILE_PACKAGE] = LoadIcon(ResPath("file-opk.png"));//m_surfaces[T_SURFACE_FILE_PACKAGE] = LoadIcon(ResPath("skin/fixit.png"));//
        m_surfaces[T_SURFACE_FILE_IS_SYMLINK] = LoadIcon(ResPath("file-is-symlink.png"));//m_surfaces[T_SURFACE_FILE_IS_SYMLINK] = LoadIcon(ResPath("skin/icon-character.png"));//
        m_surfaces[T_SURFACE_UP] = LoadIcon(ResPath("up.png"));//m_surfaces[T_SURFACE_UP] = LoadIcon(ResPath("skin/icon-delete.png"));//
    }
