...
BracketedString angleBracketed(string &s) {
  return BracketedString(s, '<', '>');
}
BracketedString angleBracketed(const string &s) {
  return BracketedString(s, '<', '>');
}
  ...
  while (getline(file, line)) {
    ...
    if (line.front() == '<') {
      ...
      while ((iss >> angleBracketed(mode))) {
        bool found =
            std::find(modeTags.begin(), modeTags.end(), mode) != modeTags.end();
        if (!found)
          modeTags.push_back(mode);
      }
    ...
    istringstream iss(line);
    if (!(iss >> quoted(id) >> quoted(description) >> quoted(options) >>
          quoted(displayValues) >> quoted(selectedValue))) {
      printErrorAndExit("cannot process line: ", line);
    }
    ...
    // create setting item
    auto item = new SettingItem(id, description, options, displayValues,
                                selectedValue, commands, infoCommand, modeTags);
    if (item->IsInitOK() == false) {
      printErrorAndExit(item->getErrorMessage() + ": ", line);
    }
  ...
  for (auto &group : settingGroups) {
    ...
    for (auto &item : group->getItems()) {
      ...
      if (item->isInfoText()) {
        ...
        file << quoted(item->getID()) << ' ' << quoted(item->getDescription())
...
void runCommands() {
  ...
  for (auto &group : settingGroups) {
    for (auto &item : group->getItems()) {
      ...
      if (index == item->getOldSelectedIndex())
        continue;
...
void handleOptions(int argc, char *argv[]) {
  ...
  while (i < argc) {
    ...
    if (strcmp(option, "-h") == 0 || strcmp(option, "--help") == 0) {
      ...
      if (mode.empty())
        printErrorUsageAndExit("-m: Invalid mode");
      bool found = std::find(modes.begin(), modes.end(), mode) != modes.end();
      if (found)
        printErrorUsageAndExit("-m: Mode alread added");
      ...
      if (page <= 0)
        printErrorUsageAndExit("-p: Invalid page index");
      ...
      titleText = argv[i + 1];
      if (titleText.empty())
        printErrorUsageAndExit("-t: Title can't ne empty");
      isShowTitle = true;
      i += 2;
    } else
...
void updateGroupNameTexture() {
  ...
  groupNameTexture = new TextTexture(
      oss.str(), global::font, global::text_color, TextureAlignment::topCenter);
}
...
void updateItemIndexTexture() {
  ...
  ostringstream oss;
  oss << group->getSelectedIndex() + 1 << '/'
      << group->getVisibleItems().size();
  itemIndexTexture =
      new TextTexture(oss.str(), global::font, global::text_color,
...
void prepareTextures() {
  ...
  SDL_Rect overlay_bg_rect = {0, 0, global::screen_width, overlay_height};
  ...
  SDL_Surface *surfacebg = SDL_CreateRGBSurface(0, global::screen_width,
                                                overlay_height, 32, 0, 0, 0, 0);
  SDL_FillRect(surfacebg, &overlay_bg_rect,
               SDL_MapRGB(surfacebg->format, 80, 80, 80));
  SDL_SetSurfaceBlendMode(surfacebg, SDL_BLENDMODE_BLEND);
  messageBGTexture = SDL_CreateTextureFromSurface(global::renderer, surfacebg);
  ...
  if (isShowTitle) {
    titleTexture = new TextTexture(titleText, global::font, global::text_color,
                                   TextureAlignment::topCenter);
  }
  instructionTexture =
      new TextTexture(instructionText, global::font, global::minor_text_color,
                      TextureAlignment::bottomLeft);
  applyingSettingsTexture =
      new TextTexture("Applying settings...", global::font, global::text_color,
                      TextureAlignment::center);
  // instructionTexture->FitScreenSize(10, 0);

  // create left and right arrow textures
  prevTexture = new TextTexture("<", global::font, global::text_color,
                                TextureAlignment::topLeft);

  nextTexture = new TextTexture(">", global::font, global::text_color,
                                TextureAlignment::topLeft);

  // create L and R button textures
  buttonLTexture =
      new TextTexture(" \u24C1 Prev", global::font, global::minor_text_color,
                      TextureAlignment::topLeft);
  buttonRTexture =
      new TextTexture("Next \u24C7 ", global::font, global::minor_text_color,
                      TextureAlignment::topRight);
}

void renderAllSettings(int offsetX = 0, bool isShowHighlight = true,
  ...
  if (isShowInstruction) {
    instructionTexture->render(0, 0);
    if (itemIndexTexture != nullptr)
      itemIndexTexture->render(-20, 0);
  }
  ...
  if (nonEmptyGroups.size() > 1) {
    if (!isShowTitle) {
      ...
      if (!isShowSinglePage)
        buttonLTexture->render(10, 10);
      if (!isShowSinglePage)
        buttonRTexture->render(-20, 10);
    } else {
      marginTop += 60;
      groupNameTexture->render(0, 0);
      if (!isShowSinglePage)
        buttonLTexture->render(60, 0);
      if (!isShowSinglePage)
  ...
  if (topItemIndex > selectedItemIndex)
  ...
  do {
    ...
    checkNeeded = false;
    if (totalHeight > global::screen_height - instructionTexture->getHeight() &&
        topItemIndex < selectedItemIndex) {
      topItemIndex++;
      checkNeeded = true;
    }
  ...
  for (auto &item : items) {
    ...
    if (offsetY + item->getHeight() >
        global::screen_height - instructionTexture->getHeight())
      break;

    // Render background if it is selected
    if (index == selectedItemIndex && isShowHighlight) {
      auto rect = overlay_bg_render_rect;
      rect.y = offsetY;           // Correcting for 0-degree rotation
      rect.h = item->getHeight(); // Adjust height properly
    ...
    if (!item->isInfoText()) {
      if (item->isOnOffSetting()) {
        int x = offsetX + global::screen_width - marginRight -
                toggleOnTexture->getWidth();
        int y =
            offsetY + (item->getHeight() - toggleOnTexture->getHeight()) / 2;
...
void ScrollLeft() {
  ...
  while (step > 0) {
    ...
    int offsetX = static_cast<int>(-global::screen_width * easing);
    renderAllSettings(offsetX, false);

    selectedGroupIndex = next;
    renderAllSettings(offsetX + global::screen_width, false, false);
...
void ScrollRight() {
  auto curr = selectedGroupIndex;
  auto prev = selectedGroupIndex > 0 ? selectedGroupIndex - 1
                                     : nonEmptyGroups.size() - 1;
  double step = 1;
  while (step > 0) {
    SDL_RenderClear(global::renderer);
...
void keyPress(const SDL_Event &event) {
  ...
  case SDLK_TAB:
    if (!isShowSinglePage) {
      selectedGroupIndex = selectedGroupIndex > 0 ? selectedGroupIndex - 1
                                                  : nonEmptyGroups.size() - 1;
...
int main(int argc, char *argv[]) {
  ...
  global::font =
      TTF_OpenFont((resourcePath + "./nunwen.ttf").c_str(), fontSize * 2);
  ...
  SDL_Window *window =
      SDL_CreateWindow("Main", 0, 0, global::screen_width,
