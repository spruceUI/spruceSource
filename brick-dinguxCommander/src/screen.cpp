...
int Screen::init()
{
    ...
    printf("%dx%d - %dx%d, %d\n", screen.w, screen.h, screen.actual_w, screen.actual_h, cfg.disp_autoscale);
#ifndef USE_SDL2
#ifdef MIYOOMINI
    GFX_Init();
#endif
    ...
    if (cfg.disp_autoscale
        ...
        if (cfg.disp_autoscale_dpi) {
            if (best.current_w >= screen.w * 2) {
                // E.g. 1024x768. Upscale to the smaller of the two.
                const float scale
    ...
    int window_w = screen.actual_w = 1024;
    int window_h = screen.actual_h = 768;
    window = SDL_CreateWindow("Commander", SDL_WINDOWPOS_UNDEFINED,
        SDL_WINDOWPOS_UNDEFINED, window_w, window_h, /*window_flags*/SDL_WINDOW_SHOWN);
    if (window == nullptr) {
